...

{
  ...
  /**
   * Obtener estructura completa de carpetas de forma recursiva
   * Retorna árbol jerárquico con todos los niveles
   */
  public function obtenerEstructuraCompletaCarpetas($idCliente, $idPlanAccion, $idCarpetaPadre = null)
  {
    try {
      global $tabla_carpetas_drive, $tabla_archivos_drive;

      // Si no se especifica carpeta padre, obtener carpeta raíz del plan
      if ($idCarpetaPadre === null) {
        $queryRaiz = "SELECT id_carpeta_drive, nombre_carpeta, tipo_carpeta, ruta_completa, id_carpeta_padre
                      FROM $tabla_carpetas_drive 
                      WHERE id_cliente = ? AND id_plan_accion = ? 
                      AND tipo_carpeta = 'plan_accion' AND estado_activo = 1
                      LIMIT 1";
        
        $resultadoRaiz = $this->funciones->fnBuscarDatosRegistro($this->conn, $queryRaiz, [$idCliente, $idPlanAccion], 'ii');
        
        if (!$resultadoRaiz['success'] || empty($resultadoRaiz['datos'])) {
          return [
            'success' => true,
            'datos' => []
          ];
        ...
        $carpetaRaiz = $resultadoRaiz['datos'][0];
        $idCarpetaPadre = $carpetaRaiz['id_carpeta_drive'];
        
        // Construir estructura recursiva empezando desde la raíz
        $estructura = $this->construirArbolCarpetas($idCliente, $idPlanAccion, $carpetaRaiz);
        ...
        // Obtener carpeta específica y construir su árbol
        // Validar que pertenezca al cliente y plan de acción correcto
        $queryCarpeta = "SELECT id_carpeta_drive, nombre_carpeta, tipo_carpeta, ruta_completa, id_carpeta_padre
                         FROM $tabla_carpetas_drive 
                         WHERE id_carpeta_drive = ? AND id_cliente = ? AND id_plan_accion = ? AND estado_activo = 1
                         LIMIT 1";
        
        $resultadoCarpeta = $this->funciones->fnBuscarDatosRegistro($this->conn, $queryCarpeta, [$idCarpetaPadre, $idCliente, $idPlanAccion], 'sii');
        
        if (!$resultadoCarpeta['success'] || empty($resultadoCarpeta['datos'])) {
          return [
            'success' => false,
        ...
        $carpeta = $resultadoCarpeta['datos'][0];
        $estructura = $this->construirArbolCarpetas($idCliente, $idPlanAccion, $carpeta);
      ...
      error_log("Error obteniendo estructura completa de carpetas: " . $e->getMessage());
      return [
        'success' => false,
        'error' => $e->getMessage()
      ];
   ...
   */
  private function construirArbolCarpetas($idCliente, $idPlanAccion, $carpeta, $nivel = 0)
  {
    global $tabla_carpetas_drive, $tabla_archivos_drive;

    // Obtener cantidad de archivos en esta carpeta
    $queryArchivos = "SELECT COUNT(*) as total 
                      FROM $tabla_archivos_drive 
                      WHERE id_carpeta_drive = ? AND estado_activo = 1";
    $resultadoArchivos = $this->funciones->fnBuscarDatosRegistro(
      $this->conn, 
      $queryArchivos, 
      [$carpeta['id_carpeta_drive']], 
      's'
    );
    $cantidadArchivos = 0;
    if ($resultadoArchivos['success'] && !empty($resultadoArchivos['datos'])) {
      $cantidadArchivos = (int)$resultadoArchivos['datos'][0]['total'];
    }

    // Obtener subcarpetas
    // Solo incluir carpetas del mismo plan de acción para evitar fuga de datos
    $querySubcarpetas = "SELECT id_carpeta_drive, nombre_carpeta, tipo_carpeta, ruta_completa, id_carpeta_padre
                         FROM $tabla_carpetas_drive 
                         WHERE id_carpeta_padre = ? AND id_cliente = ? 
                         AND id_plan_accion = ?
                         AND estado_activo = 1
                         ORDER BY nombre_carpeta ASC";
    
    $resultadoSubcarpetas = $this->funciones->fnBuscarDatosRegistro(
      $this->conn, 
      $querySubcarpetas, 
      [$carpeta['id_carpeta_drive'], $idCliente, $idPlanAccion], 
      'sii'
    );

    $subcarpetas = [];
    if ($resultadoSubcarpetas['success'] && !empty($resultadoSubcarpetas['datos'])) {
      foreach ($resultadoSubcarpetas['datos'] as $subcarpeta) {
        $subcarpetas[] = $this->construirArbolCarpetas($idCliente, $idPlanAccion, $subcarpeta, $nivel + 1);
      }
    }
    ...
    return [
      'id_carpeta_drive' => $carpeta['id_carpeta_drive'],
      ...
      'cantidad_subcarpetas' => $cantidadSubcarpetas,
      'subcarpetas' => $subcarpetas
    ];
  }

  /**
   * Crear carpeta en cualquier nivel (modificación de crearSubcarpeta)
   */
  public function crearCarpetaEnCualquierNivel($idCliente, $idPlanAccion, $nombreCarpeta, $idCarpetaPadre = null)
  {
    try {
      // Si no se especifica carpeta padre, usar carpeta raíz del plan
      if ($idCarpetaPadre === null) {
        $carpetaRaiz = $this->obtenerCarpetaRaizPlanAccion($idCliente, $idPlanAccion);
        ...
        // Validar que la carpeta padre existe y pertenece al cliente/plan
        global $tabla_carpetas_drive;
        $queryValidar = "SELECT id_carpeta_drive, ruta_completa 
                         FROM $tabla_carpetas_drive 
                         WHERE id_carpeta_drive = ? AND id_cliente = ? 
                         AND id_plan_accion = ?
                         AND estado_activo = 1
                         LIMIT 1";
        
        $resultadoValidar = $this->funciones->fnBuscarDatosRegistro(
          $this->conn, 
          $queryValidar, 
          [$idCarpetaPadre, $idCliente, $idPlanAccion], 
          'sii'
        );
        
        if (!$resultadoValidar['success'] || empty($resultadoValidar['datos'])) {
          return [
            'success' => false,
            'error' => 'La carpeta padre no existe o no tiene permisos'
          ];
        }
      }

      // Crear carpeta en Google Drive
      $resultado = $this->crearCarpeta($nombreCarpeta, $idCarpetaPadre);

      if (!$resultado['success']) {
        return $resultado;
      }

      // Obtener ruta completa de la carpeta padre
      $rutaPadre = $this->obtenerRutaCompleta($idCarpetaPadre);
      $rutaCompleta = $rutaPadre ? $rutaPadre . '/' . $nombreCarpeta : $nombreCarpeta;

      // Guardar en BD
      $resultadoBD = $this->guardarCarpetaEnBD(
        $idCliente, 
        $resultado['id_carpeta'], 
        $nombreCarpeta, 
        $idCarpetaPadre, 
        'subcarpeta', 
        $rutaCompleta, 
      ...
      if (!$resultadoBD['success']) {
        error_log("Error guardando carpeta en BD: " . $resultadoBD['response']);
        // La carpeta ya se creó en Drive, pero no se guardó en BD
        // Podríamos intentar eliminarla de Drive, pero por ahora solo logueamos
      }
      ...
      return [
        ...
        'datos' => [
          ...
          'nivel' => $this->calcularNivelCarpeta($idCarpetaPadre) + 1
        ]
      ];
    } catch (Exception $e) {
      error_log("Error creando carpeta en cualquier nivel: " . $e->getMessage());
      return [
        'success' => false,
        'error' => $e->getMessage()
      ];
  ...
  /**
   * Calcular nivel de una carpeta basado en su padre
   */
  private function calcularNivelCarpeta($idCarpetaDrive)
  {
    try {
      global $tabla_carpetas_drive;
      
      $nivel = 0;
      $idActual = $idCarpetaDrive;
      $maxNiveles = 20; // Prevenir loops infinitos
      
      while ($idActual && $nivel < $maxNiveles) {
        $query = "SELECT id_carpeta_padre, tipo_carpeta 
                  FROM $tabla_carpetas_drive 
                  WHERE id_carpeta_drive = ? AND estado_activo = 1
                  LIMIT 1";
        
        $resultado = $this->funciones->fnBuscarDatosRegistro($this->conn, $query, [$idActual], 's');
        
        if (!$resultado['success'] || empty($resultado['datos'])) {
          break;
        }
        
        $carpeta = $resultado['datos'][0];
        
        // Si es carpeta raíz del plan, ese es el nivel 0
        if ($carpeta['tipo_carpeta'] === 'plan_accion') {
          return $nivel;
        }
        
        $idActual = $carpeta['id_carpeta_padre'];
        $nivel++;
      }
      
      return $nivel;
    } catch (Exception $e) {
      error_log("Error calculando nivel de carpeta: " . $e->getMessage());
      return 0;
    }
  }
   ...
   */
  public function obtenerSubcarpetas($idCliente, $idPlanAccion, $idCarpetaPadre)
  {
    try {
      global $tabla_carpetas_drive, $tabla_archivos_drive;

      $query = "SELECT id_carpeta_drive, nombre_carpeta, tipo_carpeta, ruta_completa, id_carpeta_padre
                FROM $tabla_carpetas_drive 
                WHERE id_carpeta_padre = ? AND id_cliente = ? 
                AND id_plan_accion = ?
                AND estado_activo = 1
      ...
      $resultado = $this->funciones->fnBuscarDatosRegistro(
        $this->conn, 
        $query, 
        [$idCarpetaPadre, $idCliente, $idPlanAccion], 
        'sii'
      ...
      // Agregar información adicional a cada carpeta
      $carpetas = [];
      foreach ($resultado['datos'] as $carpeta) {
        ...
        // Contar subcarpetas (solo del mismo plan de acción para evitar fuga de datos)
        $querySubcarpetas = "SELECT COUNT(*) as total 
                             FROM $tabla_carpetas_drive 
                             WHERE id_carpeta_padre = ? AND id_cliente = ? 
                             AND id_plan_accion = ? AND estado_activo = 1";
        $resultadoSubcarpetas = $this->funciones->fnBuscarDatosRegistro(
          $this->conn, 
          $querySubcarpetas, 
          [$carpeta['id_carpeta_drive'], $idCliente, $idPlanAccion], 
        ...
        if ($resultadoSubcarpetas['success'] && !empty($resultadoSubcarpetas['datos'])) {
          $cantidadSubcarpetas = (int)$resultadoSubcarpetas['datos'][0]['total'];
        }
      ...
      error_log("Error obteniendo subcarpetas: " . $e->getMessage());
      return [
        'success' => false,
        'error' => $e->getMessage()
      ];
