...
/**
 * Archivo AJAX para obtener la carpeta raíz de un plan de acción
 * Útil para crear la primera carpeta cuando no hay subcarpetas
 */
...
try {
  ...
  $resultado = $funcionesDrive->obtenerCarpetaRaizPlanAccion($id_cliente, $id_plan_accion);
  ...
  if ($resultado['success']) {
    ...
    $funciones->fnRegresarRespuestaJsonEncode(
      ...
      $icon_error,
      'Error',
      $resultado['error'] ?? 'No se encontró la carpeta raíz del plan',
      []
    );
  }
} catch (Exception $e) {
  error_log("Error en obtener_carpeta_raiz_plan.php: " . $e->getMessage());
