...
/**
 * Archivo AJAX para obtener los archivos de una carpeta específica
...
try {
  ...
  $id_carpeta_drive = isset($_GET['id_carpeta_drive']) ? trim($_GET['id_carpeta_drive']) : '';

  if ($id_cliente <= 0 || $id_plan_accion <= 0 || empty($id_carpeta_drive)) {
  ...
  // Consulta para obtener archivos de la carpeta específica
  $query = "
    SELECT 
      ad.id_archivo,
      ad.id_archivo_drive,
    ...
    INNER JOIN $tabla_carpetas_drive AS cd ON ad.id_carpeta_drive = cd.id_carpeta_drive
    WHERE ad.id_cliente = ? 
    AND ad.id_plan_accion = ? 
    AND ad.id_carpeta_drive = ?
    AND ad.estado_activo = 1
    ORDER BY ad.fecha_subida DESC
  ";

  $parametros = [$id_cliente, $id_plan_accion, $id_carpeta_drive];
  $tipos = 'iis';
